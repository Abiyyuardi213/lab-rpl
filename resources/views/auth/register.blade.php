        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const updateUI = (step) => {
                ...
                for (let i = 1; i <= totalSteps; i++) {
                    ...
                    if (i < step) {
                        ...
                        circle.className =
                            'step-circle bg-emerald-500 text-white shadow-lg shadow-emerald-500/20';
                        label.className =
                            'text-[10px] font-black text-zinc-900 mt-2 text-center uppercase tracking-tighter';
                    } else if (i === step) {
                        circle.innerHTML = i;
                        circle.className =
                            'step-circle bg-[#001f3f] text-white shadow-lg shadow-[#001f3f]/20 scale-110';
                        label.className =
                            'text-[10px] font-black text-[#001f3f] mt-2 text-center uppercase tracking-tighter';
                    } else {
                        circle.innerHTML = i;
                        circle.className = 'step-circle bg-white border border-zinc-200 text-zinc-400';
                        label.className =
                            'text-[10px] font-bold text-zinc-400 mt-2 text-center uppercase tracking-tighter';
                    }
                }
            ...
            window.nextStep = async (e) => {
                ...
                if (currentStep === 1) {
                    ...
                    const npmRegex = /^\d{2}\.\d{4}\.\d{1}\.\d{5}$/;
                    if (!npmRegex.test(npm)) {
                        valid = false;
                        Swal.fire({
                            icon: 'error',
                            title: 'Format NPM Salah',
    ...
    <div class="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        <div class="text-center lg:text-left space-y-8 animate-fade h-full flex flex-col justify-center">
            ...
            <div class="hidden lg:block pt-8">
                <a href="/"
                    class="inline-flex items-center gap-2 text-sm font-medium text-zinc-500 hover:text-zinc-900 transition-colors group">
            ...
            <div class="w-full max-w-lg bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden mb-6">
                <div class="p-8 lg:p-10">
                    ...
                    <div class="text-center mb-10">
                        <span id="stage-text"
                            class="px-4 py-1.5 rounded-full bg-zinc-100 border border-zinc-200 text-[#001f3f] text-[10px] font-black uppercase tracking-widest shadow-sm">Langkah
                            Ke-1</span>
                    </div>
                    ...
                    <form action="{{ route('register.post') }}" method="POST" id="register-form">
                        ...
                        <div id="step-1" class="step-content space-y-5">
                            <div class="space-y-2">
                                <label for="npm"
                                    class="block text-sm font-bold text-zinc-700 uppercase tracking-tight">Nomor Pokok
                                    Mahasiswa (NPM)</label>
                                <div class="relative group input-group-custom">
                                    <span
                                        class="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 flex items-center justify-center text-zinc-400 group-focus-within:text-[#001f3f] transition-colors z-10">
                                    ...
                                    <input type="text" name="npm" id="npm" value="{{ old('npm') }}"
                                        ...
                                        placeholder="Contoh: 06.2025.1.01111" required autofocus
                                        pattern="\d{2}\.\d{4}\.\d{1}\.\d{5}" title="Format NPM harus: 06.2025.1.01111">
                                </div>
                            </div>
                        ...
                        <div id="step-2" class="step-content space-y-5 hidden">
                            <div class="space-y-2">
                                <label for="name"
                                    class="block text-sm font-bold text-zinc-700 uppercase tracking-tight">Nama
                                    Mahasiswa</label>
                                <div class="relative group input-group-custom">
                                    <span
                                        class="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 flex items-center justify-center text-zinc-400 group-focus-within:text-[#001f3f] transition-colors z-10">
                            ...
                            <div class="space-y-2">
                                <label for="email"
                                    class="block text-sm font-bold text-zinc-700 uppercase tracking-tight">Email
                                    Kampus</label>
                                <div class="relative group input-group-custom">
                                    <span
                                        class="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 flex items-center justify-center text-zinc-400 group-focus-within:text-[#001f3f] transition-colors z-10">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div class="space-y-2">
                                    <label for="password_confirmation"
                                        class="block text-sm font-bold text-zinc-700 uppercase tracking-tight">Konfirmasi</label>
                                    <div class="relative group input-group-custom">
                                        <span
                                            class="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 flex items-center justify-center text-zinc-400 group-focus-within:text-[#001f3f] transition-colors z-10">
                            ...
                            <p class="text-[11px] font-bold text-rose-500 uppercase tracking-widest leading-tight">
                                *Password minimal 6 karakter</p>
                        ...
                        <div id="step-3" class="step-content space-y-6 hidden">
                            <div class="bg-zinc-50 rounded-xl p-5 border border-zinc-200 space-y-4 shadow-inner">
                                <div class="flex justify-between items-center group">
                                    <span
                                        class="text-[10px] font-black text-zinc-400 uppercase tracking-widest">NPM</span>
                                    <span id="sum-npm"
                                        class="text-sm font-bold text-[#001f3f] group-hover:translate-x-1 transition-transform">-</span>
                                </div>
                                <div class="h-[1px] bg-zinc-200/50"></div>
                                <div class="flex justify-between items-center group">
                                    <span
                                        class="text-[10px] font-black text-zinc-400 uppercase tracking-widest">Nama</span>
                                    <span id="sum-name"
                                        class="text-sm font-bold text-[#001f3f] group-hover:translate-x-1 transition-transform">-</span>
                                </div>
                                <div class="h-[1px] bg-zinc-200/50"></div>
                                <div class="flex justify-between items-center group">
                                    <span
                                        class="text-[10px] font-black text-zinc-400 uppercase tracking-widest">Email</span>
                                    <span id="sum-email"
                                        class="text-sm font-bold text-[#001f3f] group-hover:translate-x-1 transition-transform">-</span>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-3 p-4 bg-zinc-50 border border-zinc-100 rounded-lg">
                                ...
                                <p class="text-xs text-zinc-500 font-medium leading-tight italic">Pastikan data sudah
                            ...
                            <div class="grid grid-cols-2 gap-3 mt-4">
                                ...
                                <button type="submit"
                                    class="flex justify-center items-center gap-2 rounded-lg bg-emerald-600 px-4 py-3 text-sm font-bold text-white hover:bg-emerald-700 transition-all active:scale-[0.98] shadow-lg shadow-emerald-600/10">
                ...
                <div class="px-8 py-5 bg-zinc-50 border-t border-zinc-100 text-center">
                    <p class="text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-3">Sudah memiliki
                        akses?</p>
                    <a href="{{ route('login.praktikan') }}"
                        class="inline-flex items-center gap-2 justify-center w-full py-2.5 rounded-xl border border-zinc-200 bg-white text-xs font-black text-zinc-900 hover:bg-zinc-50 transition-all active:scale-[0.98] uppercase tracking-widest shadow-sm">
