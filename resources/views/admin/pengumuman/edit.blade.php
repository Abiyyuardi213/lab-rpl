...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex items-start justify-between">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-zinc-900">Ubah Pengumuman</h1>
                <p class="text-sm text-zinc-500 mt-1">Perbarui informasi pengumuman yang sudah ada.</p>
            </div>
            ...
            <div class="space-y-6">
                    ...
                    <div class="space-y-4">
                        <div class="grid gap-2">
                            ...
                            <select id="kategori" name="kategori" required
                                ...
                                <option value="umum"
                                    {{ old('kategori', $pengumuman->kategori) == 'umum' ? 'selected' : '' }}>Umum / Berita
                                </option>
                                <option value="praktikum"
                                    {{ old('kategori', $pengumuman->kategori) == 'praktikum' ? 'selected' : '' }}>Informasi
                                    Praktikum</option>
                                <option value="kegiatan"
                                    {{ old('kategori', $pengumuman->kategori) == 'kegiatan' ? 'selected' : '' }}>Kegiatan
                                    Lab</option>
                            </select>
                            @error('kategori')
                                <p class="text-[11px] text-rose-500 font-medium">{{ $message }}</p>
                ...
                <div class="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
                    ...
                    <div class="grid gap-4">
                        <div class="flex flex-col items-center justify-center border-2 border-dashed border-zinc-200 rounded-xl p-4 transition-all hover:bg-zinc-50/50 group @if ($pengumuman->gambar) hidden @endif"
                            id="upload-container">
                            <i
                        ...
                        <div id="image-preview"
                            class="@if (!$pengumuman->gambar) hidden @endif relative rounded-xl overflow-hidden aspect-video border border-zinc-200">
