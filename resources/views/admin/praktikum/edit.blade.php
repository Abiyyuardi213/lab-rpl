        ...
        <div class="max-w-2xl">
            <div class="rounded-xl border border-zinc-200 bg-white text-zinc-950 shadow-sm overflow-hidden">
                <div class="p-6 border-b border-zinc-100 bg-zinc-50/50">
                    <h3 class="font-bold text-zinc-900">Form Pembaruan Praktikum</h3>
                </div>
                <form action="{{ route('admin.praktikum.update', $praktikum->id) }}" method="POST" class="p-6 space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-zinc-700 uppercase tracking-tight">Status Praktikum</label>
                            <select name="status_praktikum" required
                                ...
                                <option value="open_registration"
                                    {{ $praktikum->status_praktikum == 'open_registration' ? 'selected' : '' }}>Buka
                                    Pendaftaran</option>
                                <option value="on_progress"
                                    {{ $praktikum->status_praktikum == 'on_progress' ? 'selected' : '' }}>Berlangsung
                                </option>
                                <option value="finished"
                                    {{ $praktikum->status_praktikum == 'finished' ? 'selected' : '' }}>Berakhir</option>
                            </select>
                        </div>
                    </div>
