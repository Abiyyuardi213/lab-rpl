                    ...
                    <div class="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-left">
                                <div class="space-y-2 text-left">
                                    ...
                                    <input type="text" name="angkatan" id="angkatan"
                                        value="{{ old('angkatan', $aslab->aslab?->angkatan) }}"
                                        class="flex h-10 w-full rounded-lg border border-zinc-200 bg-zinc-50/50 px-3 py-1 text-sm shadow-sm transition-all focus:bg-white focus:ring-2 focus:ring-[#001f3f]/10 focus:border-[#001f3f] outline-none"
                                        placeholder="2024">
                                    @error('angkatan')
                                        <p class="text-[10px] font-medium text-rose-500 mt-1">{{ $message }}</p>
                                ...
                                <div class="space-y-2">
                                    ...
                                    <select name="jabatan" id="jabatan"
                                        ...
                                        <option value="Anggota"
                                            {{ old('jabatan', $aslab->aslab?->jabatan) == 'Anggota' ? 'selected' : '' }}>
                                            Anggota Aslab</option>
                                        <option value="Koordinator Laboratorium"
                                            {{ old('jabatan', $aslab->aslab?->jabatan) == 'Koordinator Laboratorium' ? 'selected' : '' }}>
                                            Koordinator Laboratorium</option>
                                        <option value="Koordinator Praktikum Pemrograman Terstruktur"
                                            {{ old('jabatan', $aslab->aslab?->jabatan) == 'Koordinator Praktikum Pemrograman Terstruktur' ? 'selected' : '' }}>
                                            Koordinator Praktikum Pemrograman Terstruktur</option>
                                        <option value="Koordinator Praktikum Struktur Data"
                                            {{ old('jabatan', $aslab->aslab?->jabatan) == 'Koordinator Praktikum Struktur Data' ? 'selected' : '' }}>
                                            Koordinator Praktikum Struktur Data</option>
                                        <option value="Koordinator Praktikum Basis Data"
                                            {{ old('jabatan', $aslab->aslab?->jabatan) == 'Koordinator Praktikum Basis Data' ? 'selected' : '' }}>
                                            Koordinator Praktikum Basis Data</option>
                                        <option value="Sekretaris"
                                            {{ old('jabatan', $aslab->aslab?->jabatan) == 'Sekretaris' ? 'selected' : '' }}>
                                            Sekretaris</option>
                                        <option value="Bendahara"
                                            {{ old('jabatan', $aslab->aslab?->jabatan) == 'Bendahara' ? 'selected' : '' }}>
                                            Bendahara</option>
                                        <option value="Admin"
                                            {{ old('jabatan', $aslab->aslab?->jabatan) == 'Admin' ? 'selected' : '' }}>
                                            Admin</option>
                                    </select>
                                    @error('jabatan')
                                        <p class="text-[10px] font-medium text-rose-500 mt-1">{{ $message }}</p>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="space-y-2">
                                    <label for="password"
                                        class="text-sm font-bold text-zinc-700 uppercase tracking-tight">Password
                                        Baru</label>
                                    <div class="relative">
                    ...
                    <div class="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
                        ...
                        <div class="flex flex-col items-center gap-6">
                            <div id="preview-container-premium"
                                class="relative h-60 w-48 rounded-none border-2 {{ $aslab->aslab && $aslab->aslab->profile_image ? 'border-solid border-[#001f3f]/10' : 'border-dashed border-zinc-200' }} bg-zinc-50/50 flex items-center justify-center overflow-hidden group transition-all hover:border-[#001f3f]/20 hover:bg-white">
                                <div id="placeholder-text-premium"
                                    class="{{ $aslab->aslab && $aslab->aslab->profile_image ? 'hidden' : '' }} text-center p-4">
                                    <i
                                ...
                                @if ($aslab->aslab && $aslab->aslab->profile_image)
                                    <img id="image-preview-premium" src="{{ asset('storage/' . $aslab->aslab->profile_image) }}"
                                        class="h-full w-full object-cover">
                                @else
                                ...
                                <button type="button" onclick="removeImagePremium()" id="remove-btn-premium"
                                    class="{{ $aslab->aslab?->profile_image ? 'flex scale-100' : 'hidden scale-0' }} absolute top-2 right-2 h-7 w-7 rounded-lg bg-rose-500 text-white items-center justify-center shadow-lg hover:bg-rose-600 transition-all active:scale-90 origin-center">
