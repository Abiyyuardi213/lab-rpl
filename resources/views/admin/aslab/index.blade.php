...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex items-start justify-between">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-zinc-900">Daftar Asisten Laboratorium</h1>
                <p class="text-sm text-zinc-500 mt-1">Kelola data asisten laboratorium di sini.</p>
            </div>
            <div class="flex items-center gap-2 text-xs font-medium text-zinc-500">
                <a href="{{ route('admin.dashboard') }}" class="hover:text-zinc-900 transition-colors">Home</a>
                <span>/</span>
                <span class="text-zinc-900 font-semibold">Aslab</span>
            </div>
        </div>
        ...
        <div class="rounded-xl border border-zinc-200 bg-white text-zinc-950 shadow-sm overflow-hidden">
            <div class="p-6 pb-4 flex items-center justify-between gap-4 border-b border-zinc-100">
                <div class="flex items-center gap-2 flex-1">
                    <div class="relative max-w-sm w-full">
                        ...
                        <input type="text" id="customSearch" placeholder="Cari aslab..."
                ...
                <div class="flex items-center gap-2">
                    ...
                    <a href="{{ route('admin.aslab.create') }}" data-spa
            ...
            <div class="overflow-x-auto">
                <table id="aslabTable" class="w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-zinc-100 text-zinc-900">
                        @foreach ($aslabs as $index => $aslab)
                            <tr class="hover:bg-zinc-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4 font-medium">{{ $aslab->npm }}</td>
                                <td class="px-6 py-4">
                                        ...
                                        <div
                                            ...
                                            @else
                                                <span class="text-xs font-medium text-zinc-500 uppercase">
                                                    {{ substr($aslab->name, 0, 2) }}
                                                </span>
                                            @endif
                                        </div>
                                        <div class="flex flex-col">
                                            <span class="font-medium text-zinc-900">{{ $aslab->name }}</span>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-1">
                                        <a href="{{ route('admin.aslab.show', $aslab->id) }}" data-spa
                                        ...
                                        <form id="delete-form-{{ $aslab->id }}"
                                            ...
                                            @method('DELETE')
                                            <button type="button"
                                                onclick="confirmDelete('{{ $aslab->id }}', '{{ $aslab->name }}')"
    ...
    <script>
        $(document).ready(function() {
            ...
            if ($('#aslabTable').length > 0) {
                var table = $('#aslabTable').DataTable({
                    dom: 't<"flex flex-col sm:flex-row items-center justify-between px-6 py-4 border-t border-zinc-100"ip>',
                    language: {
                        info: "Menampilkan _START_ sampai _END_ dari _TOTAL_ data",
                        emptyTable: "<div class='py-20 flex flex-col items-center justify-center space-y-3'><div class='h-16 w-16 rounded-2xl bg-zinc-50 flex items-center justify-center'><i class='fas fa-users-slash text-2xl text-zinc-300'></i></div><div class='text-center'><p class='text-zinc-900 font-semibold'>Tidak ada data aslab tersedia</p><p class='text-zinc-500 text-xs mt-1'>Silakan tambah data aslab baru untuk melihat daftar di sini.</p></div></div>",
                        paginate: {
        ...
        function confirmDelete(id, name) {
            Swal.fire({
                ...
                text: "Data aslab " + name + " akan dihapus secara permanen!",
                icon: 'warning',
