            ...
            <div class="hidden lg:flex items-center space-x-6">
                ...
                @else
                    ...
                    <div class="relative group">
                        <button class="relative flex items-center gap-1.5 text-sm font-semibold transition-colors hover:text-[#001f3f] py-4 -my-4 {{ request()->is('admin/aslab*') || request()->is('admin/praktikan*') || request()->is('admin/user*') || request()->is('admin/role*') ? 'text-[#001f3f]' : 'text-slate-600' }}">
                            Data Pengguna
                            <i class="fas fa-chevron-down text-[10px] transition-transform duration-300 group-hover:rotate-180"></i>
                            <span class="absolute bottom-3 left-1/2 w-0 h-0.5 bg-[#001f3f] transition-all duration-300 transform -translate-x-1/2 group-hover:w-full {{ request()->is('admin/aslab*') || request()->is('admin/praktikan*') || request()->is('admin/user*') || request()->is('admin/role*') ? 'w-full' : '' }}"></span>
                        </button>
                        <div class="absolute left-0 top-full mt-1 w-48 bg-white border border-slate-100 rounded-xl shadow-xl py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 z-50">
                            @if (Auth::user()->role->name === 'Super Admin')
                                <a href="{{ route('admin.role.index') }}" class="block px-4 py-2 text-sm {{ request()->is('admin/role*') ? 'bg-primary/5 text-primary font-bold' : 'text-slate-600 hover:bg-slate-50 hover:text-[#001f3f]' }}">Manajemen Role</a>
                                <a href="{{ route('admin.user.index') }}" class="block px-4 py-2 text-sm {{ request()->is('admin/user*') ? 'bg-primary/5 text-primary font-bold' : 'text-slate-600 hover:bg-slate-50 hover:text-[#001f3f]' }}">Manajemen User</a>
                            @endif
                            <a href="{{ route('admin.aslab.index') }}" class="block px-4 py-2 text-sm {{ request()->is('admin/aslab*') ? 'bg-primary/5 text-primary font-bold' : 'text-slate-600 hover:bg-slate-50 hover:text-[#001f3f]' }}">Data Asisten Lab</a>
                            <a href="{{ route('admin.praktikan.index') }}" class="block px-4 py-2 text-sm {{ request()->is('admin/praktikan*') ? 'bg-primary/5 text-primary font-bold' : 'text-slate-600 hover:bg-slate-50 hover:text-[#001f3f]' }}">Data Praktikan</a>
                        </div>
                    </div>

                    <!-- Akademik Dropdown -->
                    <div class="relative group">
                        <button class="relative flex items-center gap-1.5 text-sm font-semibold transition-colors hover:text-[#001f3f] py-4 -my-4 {{ request()->is('admin/praktikum*') || request()->is('admin/jadwal-praktikum*') || request()->is('admin/pendaftaran*') || request()->is('admin/presensi*') ? 'text-[#001f3f]' : 'text-slate-600' }}">
                            Akademik
                            <i class="fas fa-chevron-down text-[10px] transition-transform duration-300 group-hover:rotate-180"></i>
                            <span class="absolute bottom-3 left-1/2 w-0 h-0.5 bg-[#001f3f] transition-all duration-300 transform -translate-x-1/2 group-hover:w-full {{ request()->is('admin/praktikum*') || request()->is('admin/jadwal-praktikum*') || request()->is('admin/pendaftaran*') || request()->is('admin/presensi*') ? 'w-full' : '' }}"></span>
                        </button>
                        <div class="absolute left-0 top-full mt-1 w-48 bg-white border border-slate-100 rounded-xl shadow-xl py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 z-50">
                            <a href="{{ route('admin.praktikum.index') }}" class="block px-4 py-2 text-sm {{ request()->is('admin/praktikum*') ? 'bg-primary/5 text-primary font-bold' : 'text-slate-600 hover:bg-slate-50 hover:text-[#001f3f]' }}">Mata Praktikum</a>
                            <a href="{{ route('admin.jadwal-praktikum.index') }}" class="block px-4 py-2 text-sm {{ request()->is('admin/jadwal-praktikum*') ? 'bg-primary/5 text-primary font-bold' : 'text-slate-600 hover:bg-slate-50 hover:text-[#001f3f]' }}">Jadwal & Sesi</a>
                            <a href="{{ route('admin.pendaftaran.index') }}" class="block px-4 py-2 text-sm {{ request()->is('admin/pendaftaran*') ? 'bg-primary/5 text-primary font-bold' : 'text-slate-600 hover:bg-slate-50 hover:text-[#001f3f]' }}">Pendaftaran</a>
                            <a href="{{ route('admin.presensi.index') }}" class="block px-4 py-2 text-sm {{ request()->is('admin/presensi*') ? 'bg-primary/5 text-primary font-bold' : 'text-slate-600 hover:bg-slate-50 hover:text-[#001f3f]' }}">Presensi</a>
                        </div>
                    </div>

                    <!-- Kegiatan & Tugas Dropdown -->
                    <div class="relative group">
                        <button class="relative flex items-center gap-1.5 text-sm font-semibold transition-colors hover:text-[#001f3f] py-4 -my-4 {{ request()->is('admin/penugasan*') || request()->is('admin/kegiatan*') ? 'text-[#001f3f]' : 'text-slate-600' }}">
                            Kegiatan
                            <i class="fas fa-chevron-down text-[10px] transition-transform duration-300 group-hover:rotate-180"></i>
                            <span class="absolute bottom-3 left-1/2 w-0 h-0.5 bg-[#001f3f] transition-all duration-300 transform -translate-x-1/2 group-hover:w-full {{ request()->is('admin/penugasan*') || request()->is('admin/kegiatan*') ? 'w-full' : '' }}"></span>
                        </button>
                        <div class="absolute left-0 top-full mt-1 w-48 bg-white border border-slate-100 rounded-xl shadow-xl py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 z-50">
                            <a href="{{ route('admin.kegiatan.index') }}" class="block px-4 py-2 text-sm {{ request()->is('admin/kegiatan*') ? 'bg-primary/5 text-primary font-bold' : 'text-slate-600 hover:bg-slate-50 hover:text-[#001f3f]' }}">Daftar Kegiatan</a>
                            <a href="{{ route('admin.penugasan.index') }}" class="block px-4 py-2 text-sm {{ request()->is('admin/penugasan*') ? 'bg-primary/5 text-primary font-bold' : 'text-slate-600 hover:bg-slate-50 hover:text-[#001f3f]' }}">Penugasan</a>
                        </div>
                    </div>

                    <!-- Informasi Dropdown -->
                    <div class="relative group">
                        <button class="relative flex items-center gap-1.5 text-sm font-semibold transition-colors hover:text-[#001f3f] py-4 -my-4 {{ request()->is('admin/pengumuman*') || request()->is('admin/notifications*') ? 'text-[#001f3f]' : 'text-slate-600' }}">
                            Informasi
                            <i class="fas fa-chevron-down text-[10px] transition-transform duration-300 group-hover:rotate-180"></i>
                            <span class="absolute bottom-3 left-1/2 w-0 h-0.5 bg-[#001f3f] transition-all duration-300 transform -translate-x-1/2 group-hover:w-full {{ request()->is('admin/pengumuman*') || request()->is('admin/notifications*') ? 'w-full' : '' }}"></span>
                        </button>
                        <div class="absolute right-0 top-full mt-1 w-48 bg-white border border-slate-100 rounded-xl shadow-xl py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 z-50">
                            <a href="{{ route('admin.pengumuman.index') }}" class="block px-4 py-2 text-sm {{ request()->is('admin/pengumuman*') ? 'bg-primary/5 text-primary font-bold' : 'text-slate-600 hover:bg-slate-50 hover:text-[#001f3f]' }}">Pengumuman</a>
                            <a href="{{ route('admin.notifications.create') }}" class="block px-4 py-2 text-sm {{ request()->is('admin/notifications*') ? 'bg-primary/5 text-primary font-bold' : 'text-slate-600 hover:bg-slate-50 hover:text-[#001f3f]' }}">Kirim Notifikasi</a>
                        </div>
                    </div>
                @endif
            </div>
                ...
                <div class="relative group">
                    ...
                    <div class="absolute right-0 mt-2 w-[320px] bg-white rounded-xl shadow-xl border border-slate-100 hidden group-hover:block z-50 animate-in fade-in slide-in-from-top-2"
                         ...
                         <div class="px-4 py-3 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 rounded-t-xl">
                             ...
                             @if(Auth::user()->unreadNotifications->count() > 0)
                                <a href="{{ route('notifications.markAllAsRead') }}" class="text-[10px] font-bold text-[#001f3f] hover:underline">Tandai semua dibaca</a>
                             @endif
                         </div>
                         <div class="max-h-80 overflow-y-auto w-full scrollbar-thin scrollbar-thumb-slate-200">
                             @forelse(Auth::user()->unreadNotifications->take(5) as $notification)
                                <div class="px-4 py-3 border-b border-slate-50 hover:bg-slate-50 transition w-full group/notif relative">
                                    <h4 class="text-sm font-bold text-slate-900 pr-6">{{ $notification->data['title'] ?? 'Info' }}</h4>
                                    ...
                                    <a href="{{ route('notifications.markAsRead', $notification->id) }}" class="absolute right-3 top-3 text-slate-300 hover:text-[#001f3f] opacity-0 group-hover/notif:opacity-100 transition" title="Tandai dibaca">
                         ...
                         @if(Auth::user()->notifications->count() > 0)
                             <div class="px-4 py-2 border-t border-slate-100 text-center bg-slate-50/50 rounded-b-xl">
                                 <a href="{{ route('notifications.index') }}" class="text-[10px] font-bold text-slate-500 hover:text-[#001f3f]">Lihat semua notifikasi ({{ Auth::user()->unreadNotifications->count() }})</a>
                             </div>
                         @endif
                    </div>
                </div>
...
<script>
    ...
    let latestNotificationId = '{{ Auth::user()->unreadNotifications->first() ? Auth::user()->unreadNotifications->first()->id : '' }}';
