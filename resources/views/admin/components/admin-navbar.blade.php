            ...
            <div class="hidden lg:flex items-center space-x-6">
                ...
                @else
                    ...
                    <a href="{{ route('admin.penugasan.index') }}"
                        class="relative group text-sm font-semibold transition-colors hover:text-[#001f3f] {{ request()->is('admin/penugasan*') ? 'text-[#001f3f]' : 'text-slate-600' }}">
                        Penugasan
                        <span
                            class="absolute -bottom-1 left-1/2 w-0 h-0.5 bg-[#001f3f] transition-all duration-300 transform -translate-x-1/2 group-hover:w-full {{ request()->is('admin/penugasan*') ? 'w-full' : '' }}"></span>
                    </a>

                    <a href="{{ route('admin.kegiatan.index') }}"
                        class="relative group text-sm font-semibold transition-colors hover:text-[#001f3f] {{ request()->is('admin/kegiatan*') ? 'text-[#001f3f]' : 'text-slate-600' }}">
                        Kegiatan
                        <span
                            class="absolute -bottom-1 left-1/2 w-0 h-0.5 bg-[#001f3f] transition-all duration-300 transform -translate-x-1/2 group-hover:w-full {{ request()->is('admin/kegiatan*') ? 'w-full' : '' }}"></span>
                    </a>
                @endif
            </div>
